...
def df_tokens_4_agree_and_different_GT():
    ...
    df_pos_exp['nb_votes_majority_token'] = df_pos_exp['token_votes'].apply(
        lambda x: ast.literal_eval(x)['nb_votes_majority_token'])
    df_pos_exp['equals_GT'] = df_pos_exp['token_votes'].apply(
        lambda x: ast.literal_eval(x)['is_majority_token_equals_gt'])
    df_pos_exp[
...
def df_tokens_3_agree_2_unique_and_different_GT():
    ...
    df_pos_exp['nb_votes_majority_token'] = df_pos_exp['token_votes'].apply(
        lambda x: ast.literal_eval(x)['nb_votes_majority_token'])
    df_pos_exp['unique_tokens'] = df_pos_exp['token_votes'].apply(lambda x: ast.literal_eval(x)['unique_tokens'])
    df_pos_exp['equals_GT'] = df_pos_exp['token_votes'].apply(
        lambda x: ast.literal_eval(x)['is_majority_token_equals_gt'])
    df_pos_exp[
