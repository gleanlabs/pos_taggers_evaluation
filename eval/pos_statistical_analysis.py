...
sns.kdeplot(data=df_pos_exp_filt, x="count_most_frequent", hue="is_most_frequent_gt")
plt.title('When a major vote equals the GT or not, distributions of counting votes(filt)')
plt.show()

# with uniques filt
g = sns.FacetGrid(data=df_pos_exp_filt, col="is_most_frequent_gt", hue='count_uniques', height=5, aspect=0.5)
g = g.map(sns.kdeplot, 'count_most_frequent', shade=True)
g.add_legend()
plt.show()
